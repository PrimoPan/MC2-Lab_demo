...
export default function ContactMenu({ className = '', id, isOpen, onToggle, transparentSurface = false, wechatHref }: ContactMenuProps): JSX.Element {
  const surfaceColorClass = transparentSurface ? 'bg-transparent!' : 'bg-[#89d8d3]!';
  ...
  return (
    ...
    >
      <a
        className={`menu__btn absolute! top-0! left-0! flex! size-[50px]! cursor-pointer! items-center! justify-center! rounded-[4px]! ${surfaceColorClass} [background-image:linear-gradient(315deg,#1f2029_0%,#463558_74%)]! text-center! opacity-80! [box-shadow:0_0_20px_0_rgba(0,0,0,0.15)]! [transition:transform_300ms]!`}
        href={`#${id}`}
